...
class JapanHouseEventFetcher:
    ...
    def _parse_html_for_json(self, html_content):
        try:
            ...
            logger.error(f"Error parsing HTML content for {
                         self.event_source}: {parsing_error}")
            self.events_data = []
    ...
    def _extract_events(self):
        ...
        try:
            ...
            for category in self.categories:
                logger.debug(f"Extracting events for category: {
                             self.categories}")
            ...
            logger.error(f"Error extracting events from {
                         self.event_source}: {extract_events_error}")
            return []
